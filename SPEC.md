...
| Price source | `yahoo-finance2` (equities/ETFs/FX) + CoinGecko (crypto, EUR-native) + Financial Times (mutual-fund NAVs by ISIN, per-asset `priceSource='ft'` override) + TradingView (dormant fallback for stocks/ETFs Yahoo misses, and manual `priceSource='tradingview'` override by `EXCHANGE:TICKER` for listings Yahoo can't quote at all) |
...
| `/assets/[id]` | Detalle de activo — gráfica de precio EUR con selector de rango: ventana de propiedad (área sólida, solo valoraciones con posición) + **cola de mercado** (línea discontinua desde `price_history` para fechas sin posición — un activo `isActive` sigue cotizando en el cron aunque esté vendido; la cola es solo visual, nunca escribe valoraciones ni toca otros cómputos), marcadores de compra/venta, KPIs, lotes FIFO abiertos, «si vendieras hoy» (delta marginal foral), P&L realizado, dividendos y composición sectores/regiones. Reactivar un activo (`updateAsset` con `isActive: true`) dispara en segundo plano un gap-fill de `price_history` + `fx_rates` del periodo desactivado vía `fetchHistory` del provider efectivo (TradingView no tiene histórico → se omite) y reconstruye sus valoraciones con el motor canónico (solo días con posición), con evento de auditoría `reactivation_backfill`. |
...
| `/statement` | Visual portfolio statement — value chart with range selector, reparto por tipo de activo, composición por regiones geográficas (JustETF), riesgo (caída desde máximos + volatilidad), composición por objetivos (1/3) y por sectores (2/3, Yahoo), value by account, P&L by type, costes (comisiones + TER), accounts table, **desglose por activo** (tabla agrupada por tipo con logo, cantidad, precio, valor, plusvalía y rentabilidad por ventana 1m/3m/6m/YTD/1a; badge «Precios a HH:MM» que pasa a aviso si la sincronización tiene más de 36h, y un punto de retraso por línea cuando el precio de ese activo concreto es anterior a la fecha de cabecera del extracto). Export PDF / XLSX / CSV / MD via `/api/exports/statement?format=`; el param opcional `asOf=YYYY-MM-DD` (validado, no futuro) reconstruye el extracto a esa fecha — replay del ledger para cantidades/coste (misma media ponderada que el recompute), última valoración ≤ fecha como precio y efectivo = opening + movimientos ≤ corte. |
...
**Asset** — `id`, `name`, `assetType` (etf / stock / bond / crypto / fund / cash-equivalent / other), `subtype` (optional), `symbol`, `ticker`, `isin`, `exchange`, `providerSymbol` (Yahoo Finance symbol override), `tradingviewSymbol` (TradingView `EXCHANGE:TICKER` symbol, e.g. `BME:AMP`; **required** when `priceSource='tradingview'` — the create-asset schema rejects the combination without it — and resolved together with `logoUrl` by `pnpm backfill:tv`), `logoUrl` (asset logo rendered by the `AssetLogo` primitive across assets/overview/watchlist/transacciones, falls back to initials when absent), `priceSource` (null = provider by type; `'ft'` routes quotes to Financial Times by ISIN; `'tradingview'` routes to the TradingView scanner by `tradingviewSymbol` — a manual override, not just the dormant Yahoo-miss fallback; see §6), `assetClassTax` (clase fiscal: decide la ventana antiaplicación 2 vs 12 meses y el bloque M720/M721; se infiere al crear el activo y se puede recalcular con `pnpm backfill:asset-class`), `currency`, `ter` (annual %), `objectiveId`, `excludeFromObjectives` (leave out of the allocation plan), `isActive`, `isWatchlisted` (starred for the Watchlist + intraday refresh), `notes`.

**WatchlistQuote** (one per watchlisted asset, upserted) — `id`, `assetId`, `price`, `currency`, `asOf`, `source` (`yahoo`/`coingecko`/`tradingview`), `updatedAt`. Last-write-wins intraday cache filled by the `sync-watchlist` cron (≈5 min). **Not a time series and never written to `price_history`** — the daily 23:00 close stays the only source of truth for history, valuations, and long-range charts.
...
**AssetTransaction** — `id`, `accountId`, `assetId`, `transactionType` (buy / sell / dividend / fee / transfer_out / split), `tradedAt`, `quantity`, `unitPrice`, `tradeCurrency`, `fxRateToEur`, `tradeGrossAmount`, `tradeGrossAmountEur`, `cashImpactEur`, `feesAmount`, `feesAmountEur`, `netAmountEur`, `dividendGross`, `dividendNet`, `withholdingTax`, `settlementDate`, `splitNumerator` / `splitDenominator` (solo filas `split`: canje N nuevas por M antiguas — `newQty = oldQty × N / M`), `linkedTransactionId`, `externalReference`, `rowFingerprint` (import dedup), `source` (`manual` / `degiro` / `binance` / `cobas`), `notes`, `rawPayload`. `transfer_out` retira unidades a custodia externa (consume FIFO sin par fiscal ni caja). `split` (acción `createAssetSplit`, modal «Registrar split») re-escala la posición entera a las 00:00 UTC de su fecha efectiva: los lotes FIFO conservan `acquiredAt`, coste bruto y comisiones (canje fiscalmente neutro), `cashImpactEur = 0` (TWR/XIRR sin flujo) y todos los cursores de cantidad (posición, valoraciones, daily balances, serie viva, custodia M720) aplican el factor en orden cronológico.
...
The full visual portfolio statement (force-dynamic, always recomputed). Sections: value chart with range selector; **reparto por tipo de activo** (donut); **composición por regiones** geográficas (donut); **riesgo** (max drawdown + annualised volatility from the 100-anchored performance index, `src/lib/risk.ts`); **composición por objetivos** (1/3, donut coloured per objective) alongside **composición por sectores** (2/3); value by account; P&L by type; **costes** — accumulated commissions + custody fees + forward-looking TER drag (`src/server/costs.ts`); accounts table; **desglose por activo** (`AssetBreakdownTable`) — holdings grouped by type, one `DataTable` per group with logo (`AssetLogo`, fallback to initials), quantity, price, market value, P&L and, per row, price return over five windows (1m/3m/6m/YTD/1a) computed on-read by `getPeriodReturns` (`src/server/returns.ts`) off the `asset_valuations.unitPriceEur` series — a not-money-weighted price return, distinct from the account-level XIRR; a header **freshness badge** («Precios a HH:MM», `report.pricesAsOfAt` = max `pricedAt` across the day's valuations) switches from `success` to `warning` past 36h stale; each asset row additionally carries its own **lag dot** when that specific asset's last valuation predates the statement's header date, since a single stale symbol (e.g. a paused TradingView listing) doesn't move the aggregate freshness badge; **Inmuebles** — one line per property (valor, valorado a, hipoteca pendiente, equity), rendered only when at least one property exists, in the on-screen statement, the MD export and the PDF export alike. `StatementTotals.realEstateEquityEur` folds into `netWorthEur` the same way it does on the overview. Exports PDF / XLSX / CSV / MD.
...
**TradingView** (`src/lib/pricing/tradingview.ts`) plays two roles, both resolved through the same `providerFor`/`providerForAsset` selection. As a **dormant fallback**, the daily sync and the watchlist cron retry any stock/ETF whose Yahoo lookup failed this run through the TradingView scanner IF it has a `tradingviewSymbol` set — the rescued quote is still written under the asset's CANONICAL (Yahoo) `price_history.symbol`, so the series stays a single one and a healthy day makes zero TradingView requests. As a **manual override**, set `priceSource='tradingview'` for a listing Yahoo can't quote at all; the asset is then resolved and quoted PRIMARILY against `tradingviewSymbol` (`EXCHANGE:TICKER`, e.g. `BME:AMP` — required by the create-asset validation once the override is chosen, and exposed as its own field in `CreateAssetModal`) and never enters the Yahoo batch, in either the daily sync or the intraday watchlist. Same trade-off as `ft`: flipping the override on an asset that already has Yahoo-sourced history **forks** `price_history` onto the new `EXCHANGE:TICKER` symbol key — the old series is preserved, but long-range charts keyed on `price_history.symbol` show a gap at the fork point (period returns are immune: they read `asset_valuations` by `assetId`) until enough TradingView history accumulates going forward. TradingView exposes no history endpoint (`fetchHistory` always rejects), so only future quotes are affected, never backfills. `pnpm backfill:tv` resolves `tradingviewSymbol` + `logoUrl` for both roles (idempotent — skips assets already resolved unless `--force`).
...
- **Boot catch-up after host downtime**: cron does not replay runs missed while the machine was off, so `finances-service.sh` spawns `scripts/catchup-prices.ts` (`pnpm catchup:prices`) in the background on every service start. Freshness-gated — if `max(price_history.pricedDateUtc)` >= yesterday **in Europe/Madrid days** (a 00:30 CEST boot is still "yesterday" in UTC and would mask a just-missed run; predicate in `src/lib/price-staleness.ts`), it exits without touching the network, so normal deploy restarts are no-ops. When stale it FIRST runs the per-asset gap-fill (same orchestrator as asset reactivation, audit action `downtime_catchup`) to recover the missed days from provider history — triggering the sync first would write today's rows and falsely close the gap-detection window (`max(date)+1`) — and THEN, once the server answers, hits this route to capture the current day's live quotes. `--force` bypasses the gate; both phases are idempotent. Logs to `~/.finances/logs/catchup.log`.
...
ADVISOR_PROFILE_MAX_BYTES=8192
